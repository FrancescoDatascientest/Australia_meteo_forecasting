...
##Grap describe MOSTACHE
plt.figure(figsize=(15, 10))
df3 = df.select_dtypes(include=['number'])
df3.boxplot(flierprops=dict(marker='o', markersize=8, markerfacecolor='red'))
plt.xticks(rotation=90, fontsize=15)
plt.yticks(fontsize=15)
plt.xlabel('Variables Numériques', fontsize=20)
...
esp=0.5
plt.bar(indices, df2.No, width=esp-0.1)
plt.bar(indices+esp, df2.Yes, width=esp-0.1)
...
## Grap Bars yes/no rain porcentual
df2_porc = df2.copy()
df2_porc.No = df2.No/(df2.No+df2.Yes)*100
df2_porc.Yes = df2.Yes/(df2.No+df2.Yes)*100
df2_porc = df2_porc.sort_values(by='Yes')

plt.figure(figsize=(17, 9))
plt.bar(indices, df2_porc.No, width=esp-0.1, color='orange', label='No')
plt.bar(indices+esp, df2_porc.Yes, width=esp-0.1, color='royalblue', label='Yes')  # Azul un poco más oscuro
plt.xticks(indices + esp / 2, df2_porc.index, rotation=90, fontsize=13)  # Centrar etiquetas
plt.legend(title='RainTomorrow', loc='upper right', fontsize=16)
plt.xlabel('Villes', fontsize=15)
plt.ylabel('Pourcentage des journées (%)', fontsize=15)
plt.title('Nombre de journées sèches et pluvieuses (=RainTomorrow) par Location', fontsize=14)  # Título agregado
plt.ylim(0, 100)
...
### evaluer la relation entre las "RainTomorrow" de chaque ville
df.head()
